...
function nn.JacobianDebug.backward(module, input, param, dparam)
   local doparam = 0
   if param then
      doparam = 1
   end
...
function nn.JacobianDebug.backwardUpdate(module, input, param)
...
function nn.JacobianDebug.forward(module, input, param)
   param = param or input
   -- perturbation amount
   local small = 1e-6
   -- 1D view of input
...
function nn.JacobianDebug.forwardUpdate(module, input, param)
   -- perturbation amount
   local small = 1e-6
   -- 1D view of input
   --local tst = param:storage()
...
function nn.JacobianDebug.testJacobian (module, input, minval, maxval)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   local jac_fprop = nn.JacobianDebug.forward(module,input)
   local jac_bprop = nn.JacobianDebug.backward(module,input)
   local error = jac_fprop-jac_bprop
   return error:abs():max()
end

function nn.JacobianDebug.testJacobianParameters (module, input, param, dparam, minval, maxval)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   param:copy(torch.rand(param:nElement()):mul(inrange):add(minval))
   local jac_bprop = nn.JacobianDebug.backward(module, input, param, dparam)
   local jac_fprop = nn.JacobianDebug.forward(module, input, param)
   local error = jac_fprop - jac_bprop
   return error:abs():max()
end

function nn.JacobianDebug.testJacobianUpdateParameters (module, input, param, minval, maxval)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   param:copy(torch.rand(param:nElement()):mul(inrange):add(minval))
   local params_bprop = nn.JacobianDebug.backwardUpdate(module, input, param)
   local params_fprop = nn.JacobianDebug.forwardUpdate(module, input, param)

   local error = params_fprop - params_bprop
   return error:abs():max()
end

function nn.JacobianDebug.testIO(module,input, minval, maxval)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
...
function nn.JacobianDebug.testAllUpdate(module, input, weight, gradWeight)
   local gradOutput
