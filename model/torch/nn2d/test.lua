...
function nntest.CMul()
   local ini = 1--math.random(3,5)
   local inj = 1--math.random(3,5)
   local ink = 3--math.random(3,5)
   local input = torch.Tensor(ini,inj,ink):zero()
   local module = nn.CMul2D(ini*inj*ink)
...
local function criterionJacobianTest1D(cri, input, target, net)
   local eps = 1e-6
   local forward = function(input, target)
      if net then 
         input = net:forward(input)
      end
      return cri:forward(input, target)
   end
   local backward = function(input, target)
      local crit_input = input
      if net then 
         crit_input = net:forward(input)
      end
      local dfdx = cri:backward(crit_input, target)
      if net then
         dfdx = net:backward(input, dfdx)
      end
      return dfdx
   end
   
   local _ = forward(input, target)
   local dfdx = backward(input, target)
   -- for each input perturbation, do central difference
   local centraldiff_dfdx = torch.Tensor():resizeAs(dfdx)
   for i=1,input:size(1) do
      -- f(xi + h)
      input[i] = input[i] + eps
      local fx1 = forward(input, target)
      -- f(xi - h)
      input[i] = input[i] - 2*eps
      local fx2 = forward(input, target)
...
function nntest.MultiCriterionTable()
   require 'MultiCriterionTable'
   require 'ForkTable'
   local input = torch.rand(10)
   local target = input:clone():add(torch.rand(10))
   target = {target, target}
   local cri = nn.MultiCriterionTable()
   cri:add(nn.MSECriterion(), 1)
   cri:add(nn.MSECriterion(), 5)
   local net = nn.Sequential()
   net:add(nn.ForkTable())
   net:get(1):add(nn.Linear(10,10))
   net:get(1):add(nn.Identity())
   criterionJacobianTest1D(cri, input, target, net)
end
...
if not nn then
   ...
   jac = nn.JacobianDebug
   sjac = nn.SparseJacobian
   function nn2d.test(tests)
      -- randomize stuff
